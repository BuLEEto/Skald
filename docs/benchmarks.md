...
Apple M4. Idle memory on Linux is ~77 MB for every app (atlas +
Vulkan buffers dominate); under the default runa backend that grows
by ~2.6 MB one-time for parsed font tables + the shape cache. Either
way, steady-state growth is flat after warmup. RSS on macOS is blank
for now — reader is Linux-only.

The pure-Odin runa backend (the default since 1.0) adds ~2.6 MB of
one-time overhead at startup (parsed font tables + shape cache) and
lands *faster* than the legacy fontstash backend on every workload —
the 00_gallery ceiling test runs at 1.22 ms / 821 fps vs 2.92 ms /
343 fps for fontstash, 2.4× faster. The shape cache (zero-alloc on
hit) is doing the work. Build with `-define:SKALD_RUNA=false` to
re-measure on the fontstash path if needed.

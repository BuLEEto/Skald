...
### Responsive layout — swap based on width

Skald doesn't have CSS media queries. Read `ctx.renderer.fb_size` and
branch inside `view`:

```odin
if f32(ctx.renderer.fb_size.x) < 900 {
    return mobile_layout(ctx, s)
}
return desktop_layout(ctx, s)
```

Cheap: you're already rebuilding the tree every frame.
...
### Clear / search input with an × button

```odin
skald.text_input(ctx, s.query, on_query,
    placeholder = "Search",
    show_clear  = true,
    on_clear    = on_clear_query)
```

The × button only renders when the field has text. Give it an `on_clear`
callback that emits a Msg for the clear action.
...
if skald.system_theme() == .Light { theme = skald.theme_light() }
...
The callback fires when the user flips their OS-level appearance so
you can mirror it without a relaunch.

...
| `51_text_styles` | `text_input` `styles`: restyle byte ranges of the editable buffer — live keyword highlighting (bold + colour) and `# `/`## ` headings (per-run size → taller lines). Caret/selection/clicks measure per run, so styled metrics stay exact. |

...
view :: proc(s: State, ctx: ^skald.Ctx(Msg)) -> skald.View {
	...
	return skald.col(
		...
		skald.radio_group(ctx,
			...
			int(s.theme), on_theme, direction = skald.Stack_Direction.Row,
		...
		// staged for OK/Apply. `disabled` flips every input in this
		// section into a muted, non-interactive state — the "Lock
		// preferences" toggle below drives it.
		section_heading("Connectivity", ctx),
		skald.spacer(th.spacing.xs),
		skald.toggle(ctx, s.wifi,      "Wi-Fi",         on_wifi,      disabled = s.locked),
		skald.spacer(th.spacing.xs),
		skald.toggle(ctx, s.bluetooth, "Bluetooth",     on_bluetooth, disabled = s.locked),
		skald.spacer(th.spacing.xs),
		skald.toggle(ctx, s.airplane,  "Airplane mode", on_airplane,  disabled = s.locked),
		skald.spacer(th.spacing.lg),

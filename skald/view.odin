...
//     5. feature/behavior params          — step, min_value, read_only, etc.
...
//     - `read_only: bool` (not `disabled`). Read-only widgets still render
...
// Pass `read_only = true` to render as a static badge with no hit-testing.
//
...
rating :: proc(
	...
	for i in 0..<max_value {
		...
		// Per-slot id so hover rects track each star. The high bit
		// pushes these out of the positional auto-id range — without
		// it, `base ~ (i+1)` can collide with a neighboring widget's
		// auto-id (e.g. base=10, i=1 → 8, which might be some other
		// widget earlier in the tree).
		slot_id := (Widget_ID(u64(base) ~ u64(i + 1))) | WIDGET_ID_EXPLICIT_BIT
...
accordion :: proc(
	...
	for sec, i in sections {
		...
		// Per-header zone id. Derive from base + i (explicit bit so it
		// can't collide with other widgets' auto-ids — same pattern the
		// rating stars use).
		zone_id := (Widget_ID(u64(base_id) ~ u64(i + 1))) | WIDGET_ID_EXPLICIT_BIT
...
flex :: proc(weight: f32, child: View) -> View {
	c := new(View, context.temp_allocator)
	c^ = child
	return View_Flex{weight = weight, child = c}
}
...
// `Input.text`.
//
// `read_only`: display-only mode. No caret, no selection, no keypress
...
text_input :: proc(
	...
	if read_only { fg_c = th.color.fg_muted }
	...
	id := widget_resolve_id(ctx, id)
	if !read_only { widget_make_focusable(ctx, id) }
	st := widget_get(ctx, id, .Text_Input)
	focused := !read_only && widget_has_focus(ctx, id)
	...
	hovered := !read_only && rect_hovered(ctx, st.last_rect)
	...
	show_clear    := search && !read_only && len(new_value) > 0
	...
	sb_captured := false
	sb_hover    := false
	if multiline && !read_only && st.last_rect.w > 0 && st.last_rect.h > 0 {
	...
	if !read_only && !sb_captured && !clear_captured && ctx.input.mouse_pressed[.Left] {
		if hovered {
			widget_focus(ctx, id)
			focused = true
...
search_field :: proc(
	...
	if widget_has_focus(ctx, id) && .Enter in ctx.input.keys_pressed {
		send(ctx, on_submit())
	}
...
checkbox :: proc(
	...
	if read_only { cfill = th.color.fg_muted }
	fg_c := th.color.fg
	if read_only { fg_c = th.color.fg_muted }
	...
	if !read_only {
		...
		if ctx.input.mouse_released[.Left] {
			if st.pressed && hovered { send(ctx, on_change(!checked)) }
			st.pressed = false
		}
		if !ctx.input.mouse_buttons[.Left] { st.pressed = false }
...
radio :: proc(
	...
	if read_only { cdot = th.color.fg_muted }
	fg_c := th.color.fg
	if read_only { fg_c = th.color.fg_muted }
	...
	if !read_only {
		...
		if ctx.input.mouse_released[.Left] {
			if st.pressed && hovered && !selected { send(ctx, on_select()) }
			st.pressed = false
		}
		if !ctx.input.mouse_buttons[.Left] { st.pressed = false }
...
radio_group :: proc(
	...
	if focused_idx >= 0 && len(options) > 1 {
		...
		if delta != 0 {
			...
			if new_idx != selected { send(ctx, on_change(new_idx)) }
	...
	for opt, i in options {
		children[i] = radio_group_option(ctx, opt_ids[i],
			opt, i == selected, i, on_change, read_only)
	}
...
// while this handles the per-option click / Space handling that needs
// to know its own index to call `on_change`.
@(private)
radio_group_option :: proc(
	...
	if !read_only { widget_make_focusable(ctx, opt_id) }
	st := widget_get(ctx, opt_id, .Radio)
	focused := !read_only && widget_has_focus(ctx, opt_id)
	hovered := !read_only && rect_hovered(ctx, st.last_rect)
	...
	if !read_only {
		...
		if ctx.input.mouse_released[.Left] {
			if st.pressed && hovered && !selected { send(ctx, on_change(index)) }
			st.pressed = false
		}
		if !ctx.input.mouse_buttons[.Left] { st.pressed = false }
		if focused && !selected && .Space in ctx.input.keys_pressed {
			send(ctx, on_change(index))
		}
	} else {
		st.pressed = false
	}
	...
	fg  := th.color.fg
	if read_only { dot = th.color.fg_muted; fg = th.color.fg_muted }
...
toggle :: proc(
	...
	if read_only { con = th.color.fg_muted }
	fg_c := th.color.fg
	if read_only { fg_c = th.color.fg_muted }
	...
	if !read_only {
		...
		if ctx.input.mouse_released[.Left] {
			if st.pressed && hovered { send(ctx, on_change(!on)) }
			st.pressed = false
		}
		if !ctx.input.mouse_buttons[.Left] { st.pressed = false }
		if focused && .Space in ctx.input.keys_pressed {
			send(ctx, on_change(!on))
		}
	} else {
		st.pressed = false
	}
...
slider :: proc(
	...
	if !read_only && ctx.input.mouse_pressed[.Left] && hovered {
		st.pressed = true
		widget_focus(ctx, id)
		focused = true
	}
	...
	new_value := value
	if !read_only && st.pressed && st.last_rect.w > 0 {
	...
	if new_value != value { send(ctx, on_change(new_value)) }

	fill  := th.color.primary
	thumb := th.color.on_primary
	if read_only { fill = th.color.fg_muted; thumb = th.color.fg_muted }
...
select :: proc(
	...
	trigger_hovered := !read_only && rect_hovered(ctx, trigger_rect)
	...
	if !read_only && ctx.input.mouse_pressed[.Left] {
		if trigger_hovered {
			st.open = !st.open
			widget_focus(ctx, id)
			focused = true
	...
	for opt in options {
		...
		append(&rows, select_option_button(ctx, opt, on_change, row_bg, th))
	}
...
combobox :: proc(
	...
	if focused && !read_only {
		...
		if .Enter in keys {
			if len(visible) > 0 {
				send(ctx, on_change(options[visible[highlight]]))
				widget_focus(ctx, 0)
				focused  = false
				st.open  = false
			} else if free_form {
				send(ctx, on_change(strings.clone(draft, context.temp_allocator)))
				widget_focus(ctx, 0)
				focused  = false
				st.open  = false
			}
	...
	if st.open && ctx.input.mouse_released[.Left] && mouse_over_overlay {
		...
		idx := int(rel_y / opt_h)
		if idx >= 0 && idx < len(visible) {
			send(ctx, on_change(options[visible[idx]]))
			widget_focus(ctx, 0)
			focused = false
			st.open = false
		}
...
date_picker :: proc(
	...
	if focused && st.open && !read_only && !year_mode {
		keys  := ctx.input.keys_pressed
...
time_picker :: proc(
	...
	if ctx.input.mouse_released[.Left] {
		if prev_hover {
			...
			send(ctx, on_change(Time{hour = nh, minute = nm, second = 0}))
			st.open = false
		} else {
			rel_x := ctx.input.mouse_pos.x - content_x
			rel_y := ctx.input.mouse_pos.y - grid_origin_y
				...
				if rel_x >= 0 && rel_y >= 0 && rel_x < hour_cell_w*f32(HOUR_COLS) &&
				   rel_y < CELL_H*f32(HOUR_ROWS) {
					...
					if new_hour >= 0 && new_hour < 24 && new_hour != value.hour {
						send(ctx, on_change(Time{hour = new_hour, minute = value.minute, second = value.second}))
					}
				}
			case 1: // Minute
				   ...
				   rel_y < CELL_H*f32(min_rows) {
					...
					if idx >= 0 && idx < min_n {
						...
						if new_min != value.minute {
							send(ctx, on_change(Time{hour = value.hour, minute = new_min, second = value.second}))
						}
					}
				}
			case 2: // Second
				   ...
				   rel_y < CELL_H*f32(sec_rows) {
					...
					if idx >= 0 && idx < sec_n {
						...
						if new_sec != value.second {
							send(ctx, on_change(Time{hour = value.hour, minute = value.minute, second = new_sec}))
						}
					}
				}
			}
...
color_picker :: proc(
	...
	if !read_only && st.open { widget_make_focusable(ctx, hex_id) }
	hex_st := widget_get(ctx, hex_id, .Text_Input)
	hex_focused := !read_only && st.open && widget_has_focus(ctx, hex_id)
	...
	if !st.open { return zone_trigger }
...
segmented :: proc(
	...
	for opt, i in options {
		...
		if read_only && !is_sel { fg = th.color.fg_muted }
		if read_only && is_sel  { bg = th.color.fg_muted }

		if read_only {
			// Inert tile that matches the button's geometry so the row
			// doesn't reflow when the widget flips to/from read_only.
...
//
// `read_only` mutes the text color, drops the stepper buttons, and
...
number_input :: proc(
	...
	read_only:   bool      = false,
	disabled:    bool      = false,
	// max_chars caps the draft buffer length in runes (including the sign
	// and decimal point). 0 disables the cap. Mirrors text_input.max_chars
	// so number fields in a form feel symmetric with the text ones.
	max_chars:   int       = 0,
) -> View {
	th := ctx.theme
	base := widget_resolve_id(ctx, id)
	...
	// disabled collapses to read_only's muted-non-interactive rendering;
	// keeping two flags mirrors button/link/text_input where the distinction
	// is "read-only = show value, can't edit" vs "disabled = this control
	// is off right now." For the numeric widget both render identically; we
	// just OR them together so apps that set either get the expected feel.
	non_interactive := read_only || disabled
	...
	emit_parsed :: proc(ctx: ^Ctx($Msg), draft: string, value, min_value, max_value: f64,
	                   on_change: proc(new_value: f64) -> Msg) {
		parsed, ok := strconv.parse_f64(draft)
		if !ok { return }
		if parsed < min_value { parsed = min_value }
		if parsed > max_value { parsed = max_value }
		if parsed != value { send(ctx, on_change(parsed)) }
	}
	...
	if focused {
		...
		if draft != st.text_buffer {
			if len(st.text_buffer) > 0 { delete(st.text_buffer) }
			st.text_buffer = strings.clone(draft)
			emit_parsed(ctx, st.text_buffer, v, min_value, max_value, on_change)
		}
		st.cursor_pos = cursor
	}
	...
	if non_interactive { return field }

	dec_target := v - step
	if dec_target < min_value { dec_target = min_value }
	inc_target := v + step
	if inc_target > max_value { inc_target = max_value }

	return row(
		button(ctx, "\u2212", on_change(dec_target), id = dec_id, width = btn_w),
		spacer(gap),
		View(field),
		spacer(gap),
		button(ctx, "+", on_change(inc_target), id = inc_id, width = btn_w),
		cross_align = .Center,
	)
}
...
virtual_list :: proc(
	...
	for i in first..<last {
		...
		scope := u64(id) ~ (k * 0x9e3779b97f4a7c15)
...
table :: proc(
	...
	for i in first..<last {
		...
		k: u64 = u64(i)
		if row_key != nil { k = row_key(state, i) }
		scope   := u64(body_id) ~ (k * 0x9e3779b97f4a7c15)
		saved   := widget_scope_push(ctx, scope)

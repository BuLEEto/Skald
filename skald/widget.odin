...
// "not focusable" case (Skald uses `read_only` / `disabled` flags for
...
widget_auto_id :: proc(ctx: ^Ctx($Msg)) -> Widget_ID {
	...
	if ctx.widgets.auto_id_scope != 0 {
		return Widget_ID(ctx.widgets.auto_id_scope ~ u64(ctx.widgets.auto_id)) | WIDGET_ID_EXPLICIT_BIT
	}
...
// never registered (read_only / disabled) or the id isn't present.
//
